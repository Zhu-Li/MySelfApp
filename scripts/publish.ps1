...
# 0. 运行古籍数据生成脚本（扫描源目录、生成 index.json / catalog / book-map.json）
Write-Host "=== Step 0: Generate classics data ==="
$classicsProc = Start-Process -FilePath "node" -ArgumentList "$source\scripts\generate-classics-data.js" -NoNewWindow -Wait -PassThru
if ($classicsProc.ExitCode -ne 0) {
    Write-Host "Classics data generation failed (exit code: $($classicsProc.ExitCode))."
    exit 1
}
Write-Host "Classics data generated successfully."
Write-Host ""
...
# 2. 同步应用代码（排除 novel/classics 目录，数据文件由前面步骤已处理）
Write-Host ""
Write-Host "=== Step 2: Sync app files ==="
# /MIR 镜像同步，/IS 强制覆盖同名文件（防止时间戳一致但内容不同的情况）
$proc = Start-Process -FilePath "robocopy.exe" -ArgumentList "$source","$dest","/MIR","/IS","/XD",".git","node_modules",".qoder","scripts","$dest\novel","$dest\classics","/XF","*.zip",".gitignore","package.json","package-lock.json","nul" -NoNewWindow -Wait -PassThru
...
Copy-Item -Path "$source\package.json" -Destination "$serviceDir\package.json" -Force
...
Write-Host "package.json        -> $serviceDir\package.json"

# 3.5 安装服务端 npm 依赖（如 msedge-tts）
Write-Host ""
Write-Host "=== Step 3.5: Install server npm dependencies ==="
Push-Location $serviceDir
npm install --production --no-audit --no-fund 2>&1 | Out-Null
Pop-Location
Write-Host "npm dependencies installed in $serviceDir"
