...
### 古籍阅读
- 支持 10 大分类（佛藏、道藏、儒藏、史藏、子藏、集藏、诗藏、医藏、艺藏、易藏）共 16000+ 古籍文献
- 三级懒加载：分类概览 → 目录树 → 按需加载内容，节省带宽
- 面包屑导航支持深层目录浏览，书籍列表分页展示
- 复用小说沉浸式阅读器，支持字体/主题/朗读等全部功能
- 小说与古籍统一为「书籍」模块，Tab 切换入口
- API 流式读取源文件，7.3GB 古籍内容按需加载
...
- **邮件服务**: Web3Forms
- **书籍API**: Node.js Windows 服务 + NSSM（端口 3001），提供小说增量同步和古籍内容流式读取
...
│   ├── classics/       # 古籍阅读模块
│   │   ├── classics.js         # 协调器（分类/目录/内容加载）
│   │   ├── classics-renderer.js # 分类卡片/面包屑/书籍列表渲染
│   │   └── classics-styles.js  # 样式
│   ├── book/           # 书籍统一模块（Tab 切换）
│   │   ├── book.js             # 协调器
│   │   ├── book-renderer.js    # Tab 栏渲染
│   │   └── book-styles.js      # 样式
│   ├── feedback/       # 意见反馈模块
│   ├── donate/         # 打赏模块
│   └── changelog/      # 更新日志模块
...
### v2.4.0
- 新增古籍阅读模块，支持浏览 10 大分类、16000+ 古籍文献
- 三级懒加载：分类概览 → 目录树 → 按需加载内容，节省带宽
- 古籍复用小说沉浸式阅读器，支持字体/主题/朗读等全部功能
- 小说与古籍合并为「书籍」模块，Tab 切换统一入口
- 数据导出新增古籍阅读进度选项
- API 服务新增古籍内容流式读取端点
